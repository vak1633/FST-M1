...
SELECT customer_id, MAX(purchase_amount) AS Maxamount FROM orders GROUP BY customer_id ORDER BY customer_id;
-- Write an SQL statement to find the highest purchase amount on '2012-08-17' for each salesman with their ID.
SELECT salesman_id, order_date, MAX(purchase_amount) AS Maxamount FROM orders WHERE order_date=TO_DATE('2012-08-17','YYYY-MM-DD') GROUP BY salesman_id, order_date ORDER BY salesman_id; 
...
SELECT customer_id, order_date, MAX(purchase_amount) AS maxamount FROM orders GROUP BY customer_id, order_date HAVING MAX(purchase_amount) IN (2030, 3450, 5760, 6000) ORDER BY customer_id;